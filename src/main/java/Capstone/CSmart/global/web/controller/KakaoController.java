...
public class KakaoController {
    ...
    public ApiResponse<KakaoResponseDTO.SendMessageResponse> sendMessage(
        ...
        try {
            log.info("카카오톡 메시지 전송 요청: recipient={}", request.getRecipient());
            
            KakaoResponseDTO.SendMessageResponse response = kakaoService.sendMessage(request);
    ...
    @Operation(
        ...
        description = "카카오톡에서 채팅 목록을 조회합니다."
    )
    ...
    public ApiResponse<KakaoResponseDTO.ChatListResponse> getChatList(
        ...
        try {
            log.info("카카오톡 채팅 목록 조회 요청: page={}, size={}", page, size);
            ...
            KakaoResponseDTO.ChatListResponse response = kakaoService.getChatList(request);
    ...
    @Operation(
        ...
        description = "카카오톡 웹훅 서비스의 상태를 확인합니다."
    )
    @GetMapping("/health")
    public ApiResponse<Boolean> checkKakaoServiceHealth() {
        try {
            boolean isHealthy = kakaoService.isServiceHealthy();
            
            log.info("카카오톡 서비스 헬스체크: isHealthy={}", isHealthy);
